...
h+" minutes ago":1<f?"about "+f+" seconds ago":"just now";b.entities=d.entities?(d.entities.urls||[]).concat(d.entities.media||[]):[];b.tweet_raw_text=b.retweet?"RT @"+b.retweeted_screen_name+" "+d.retweeted_status.text:d.text;b.tweet_text=c([n(b.tweet_raw_text,b.entities)]).linkUser().linkHash()[0];b.tweet_text_fancy=c([b.tweet_text]).makeHeart()[0];b.tweet_text_fancy=b.tweet_text_fancy.replace(/<a/g,'<a target="_blank" ');b.user=g('<a class="tweet_user" href="{tweet_url}" target="_blank">{screen_name}</a>',
b);b.join=a.join_text?g(' <span class="tweet_join">{join_text}</span> ',b):" ";b.avatar=b.avatar_size?g('<a class="tweet_avatar" href="{user_url}"><img src="{avatar_url}" height="{avatar_size}" width="{avatar_size}" alt="{screen_name}\'s avatar" title="{screen_name}\'s avatar" border="0"/></a> ',b):"";b.time=g('<span class="tweet_time"><a href="{tweet_url}" title="view tweet on twitter" target="">{tweet_relative_time}</a></span>',b);b.text=g('<span class="tweet_text">{tweet_text_fancy}</span>',b);
b.reply_action=g('<a class="tweet_action tweet_reply" href="{reply_url}">reply</a>',b);b.retweet_action=g('<a class="tweet_action tweet_retweet" href="{retweet_url}">retweet</a>',b);b.favorite_action=g('<a class="tweet_action tweet_favorite" href="{favorite_url}">favorite</a>',b);return b}function q(d){var e='<p class="tweet_intro">'+a.intro_text+"</p>",i='<p class="tweet_outro">'+a.outro_text+"</p>",h=c('<p class="loading">'+a.loading_text+"</p>");a.loading_text&&c(d).not(":has(.tweet_list)").empty().append(h);
c.getJSON(o(),function(f){var b=c('<ul class="tweet_list">'),f=c.map(f.results||f,p),f=c.grep(f,a.filter).sort(a.comparator).slice(0,a.count);b.append(c.map(f,function(b){return"<li style='{border-bottom-style: solid;border-bottom-width:1px; border-bottom-color: #ff0033;}'>"+g(a.template,b)+"</li>"}).join("")).children("li:first").addClass("tweet_first").end().children("li:odd").addClass("tweet_even").end().children("li:even").addClass("tweet_odd");c(d).empty().append(b);a.intro_text&&b.before(e);
a.outro_text&&b.after(i);c(d).trigger("loaded").trigger(0===f.length?"empty":"full");a.refresh_interval&&window.setTimeout(function(){c(d).trigger("tweet:load")},1E3*a.refresh_interval)})}var a=c.extend({username:null,list:null,favorites:!1,query:null,avatar_size:null,count:3,fetch:null,page:1,retweets:!0,intro_text:null,outro_text:null,join_text:null,auto_join_text_default:"I said,",auto_join_text_ed:"I",auto_join_text_ing:"I am",auto_join_text_reply:"I replied to",auto_join_text_url:"I was looking at",
loading_text:null,refresh_interval:null,twitter_url:"twitter.com",twitter_api_url:"api.twitter.com",twitter_search_url:"search.twitter.com",template:"{avatar}{time}{join}{text}",comparator:function(a,c){return c.tweet_time-a.tweet_time},filter:function(){return!0}},m),k=/\b((?:https?:\/\/|www\d{0,3}[.]|[a-z0-9.\-]+[.][a-z]{2,4}\/)(?:[^\s()<>]+|\(([^\s()<>]+|(\([^\s()<>]+\)))*\))+(?:\(([^\s()<>]+|(\([^\s()<>]+\)))*\)|[^\s`!()\[\]{};:'".,<>?\u00ab\u00bb\u201c\u201d\u2018\u2019]))/gi;c.extend({tweet:{t:g}});
c.fn.extend({linkUser:j(/(^|[\W])@(\w+)/gi,'$1<span class="at">@</span><a href="http://'+a.twitter_url+'/$2">$2</a>'),linkHash:j(/(?:^| )[\#]+([\w\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u00ff\u0600-\u06ff]+)/gi,' <a href="http://'+a.twitter_search_url+"/search?q=&tag=$1&lang=all"+(a.username&&1==a.username.length&&!a.list?"&from="+a.username.join("%2BOR%2B"):"")+'" class="tweet_hashtag">#$1</a>'),makeHeart:j(/(&lt;)+[3]/gi,"<tt class='heart'>&#x2665;</tt>")});return this.each(function(d,e){a.username&&
"string"==typeof a.username&&(a.username=[a.username]);c(e).unbind("tweet:load").bind("tweet:load",function(){q(e)}).trigger("tweet:load")})}});
